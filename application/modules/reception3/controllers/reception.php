...
{	
	...
	*/
	public function general_queue($page_name)
	{
		$segment = 4;
			...
			$where = 'visit.visit_type = visit_type.visit_type_id AND visit.visit_delete = 0 AND visit_department.visit_id = visit.visit_id AND visit_department.visit_department_status = 1 AND visit.patient_id = patients.patient_id AND visit.close_card = 0 AND visit.visit_date = \''.date('Y-m-d').'\'';
	...
	*/
	...
	public function search_dependant_parent($staff_number)
	{
		if(!empty($staff_number))
		{
			$query = $this->reception_model->get_staff($staff_number);
			
			//found in our database
			if ($query->num_rows() > 0)
			{
				$query_staff = $this->reception_model->get_patient_staff($staff_number);
				...
				{
					$patient_id = $this->reception_model->insert_into_patients($staff_number,2);
				...
				if(!empty($patient_id))
				{
					...
					$response = $this->load->view('patients/dependants', $v_data, TRUE);
				}
				else
				{
					$staff_search = '';
					$response = '<div class="alert alert-danger center-align"><p>Unable to create staff in patient. Please try again</p></div>';
				}
			}
			...
			{
				$response = '<div class="alert alert-danger center-align"><p>Staff member does not exist. Please add then and then add their dependants</p></div>';
			}
		...
		{
			$response = '<div class="alert alert-danger center-align"><p>Please enter a staff number to continue</p></div>';
		}
	...
	*/
	...
	{
		$v_data["patient_id"] = $primary_key;
		$v_data['visit_departments'] = $this->reception_model->get_visit_departments2();
	    $v_data['charge'] = $this->reception_model->get_service_charges($primary_key);
		$v_data['doctor'] = $this->reception_model->get_doctor();
		$v_data['type'] = $this->reception_model->get_types();
		$v_data['patient_insurance'] = $this->reception_model->get_patient_insurance($primary_key);
		...
		$cover_balance = $patient['cover_balance'];
		
	    $v_data['patient'] = 'Surname: <span style="font-weight: normal;">'.$patient_surname.'</span> Othernames: <span style="font-weight: normal;">'.$patient_othernames.'</span> Patient Type: <span style="font-weight: normal;">'.$patient_type.'</span> Account Balance : <span style="font-weight: normal;">'.$account_balance.'</span>Cover Balance : <span style="font-weight: normal;">'.$cover_balance.'</span> <a href="'.site_url().'administration/individual_statement/'.$primary_key.'/2" class="btn btn-sm btn-primary" target="_blank" style="margin-top: 5px;">Patient Statement</a>';
	...
	public function save_visit($patient_id)
	{
		...
		else
		{
			...
			else
			{
				...
				if($visit_id > 0)
				{//echo $visit_id;die();
					//1.check that staff payment method was selected
					$visit_type = $this->input->post('visit_type');
					if($visit_type==1)
					{
						//2.check that visit-payment_method is salary deduction or care of someone
						$visit_payment_type = $this->input->post('visit_payment_type');
						if(($visit_payment_type == 3)||($visit_payment_type == 4))
						{
							//3.insert the visit details in the db
							$this->reception_model->save_staff_visit_payment_options($visit_id,$visit_payment_type);
						}
					}
				}
				...
				if($appointment_id == 0)
				{
					...
					//if dental, check that patient file no is there
	...
	public function register_dependant($patient_id, $visit_type_id, $staff_no)
	{
		...
		else
		{
			...
			if($patient_id != FALSE)
			{
				...
				$response['message'] = '<div class="alert alert-success"><a href="'.site_url().'reception/set_visit/'.$patient_id.'" class="btn btn-warning pull-right">Create Visit</a> Dependant added successfully</div>';
			...
			{
				$response['response'] = 'error';
				$response['message'] = '<div class="alert alert-danger">Could not create dependant. Please try again</div>';
			}
		}
	...
	public function appointment_list()
	{
		...
		$appointment_search = $this->session->userdata('search_appointment_patients');
	...
	{//patients.dependant_id = patients.strath_no  AND 
		$segment = 3;
		
		$dependant_search = $this->session->userdata('patient_dependants_search');
		$where = 'patients.visit_type_id = 2 AND patients.dependant_id IS NOT NULL AND patients.patient_delete = 0 AND patients.strath_no = staff.Staff_Number AND staff_dependants.staff_id = staff.staff_id';
		...
		$table = 'patients,staff,staff_dependants';
		$items = 'patients.*';
		//pagination
		$this->load->library('pagination');
	...
	*/
	public function edit_staff($patient_id)
	{
		...
		$v_data['titles'] = $this->reception_model->get_title();
		$v_data['genders'] = $this->reception_model->get_gender();
		$v_data['patient_file_number' ] = $this->reception_model->get_patient_file_number($patient_id);
	...
	*/
	...
	public function edit_staff_dependant_patient($patient_id)
	{
		...
		$this->form_validation->set_rules('file_number', 'File number', 'xss_clean');
		...
		$v_data['genders'] = $this->reception_model->get_gender();
		$v_data['patient'] = $this->reception_model->get_patient_data($patient_id);
		$v_data['patient_file_number'] =$this->reception->get_patient_file_number($patient_id);
	...
	public function close_general_queue_search($page_name = NULL)
	{
	...
	public function search_appointment_patients()
	{
		...
		{
			$visit_date = ' AND visit.visit_date = "'.$appointment_date.'"';
		}
		//search surname
		if(!empty($_POST['surname']))
		{
		...
		$search = $visit_type_id.$strath_no.$surname.$other_name.$visit_date;
		$this->session->set_userdata('search_appointment_patients', $search);
		
		$this->appointment_list();
	}
	public function close_appointment_search()
	{
		$this->session->unset_userdata('search_appointment_patients');
		$this->appointment_list();
	}
	public function reopen_todays_visits()
	{
		$today = date('Y-m-d');
		//get all visits that have been closed today
		$todays_closed_visits = $this->reception_model->get_todays_closed_visits();
		$v_data['query'] = $todays_closed_visits;
		$v_data['title'] = $data['title'] = "Reopen Visits for Today ".date('jS M Y',strtotime($today));
		
		$data['content'] = $this->load->view('reception/reopen_visit',$v_data, TRUE);
		$this->load->view('admin/templates/general_page', $data);
	}
}