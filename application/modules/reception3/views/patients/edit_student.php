...
$gender = $row->gender;
$DOB = $row->DOB;
$intake_id = $row->student_intake_id;
if($gender == 'F')
{
	$gender_name = 'Female';
}
...
<div class="row">
	<div class="col-md-6">
		<a href="<?php echo site_url();?>reception/students" class="btn btn-sm btn-primary ">Back to student list</a>
	</div>
</div>
...
<div class="row">
	<div class="col-md-6">
        ...
        <div class="form-group">
            ...
            <div class="col-lg-8">
            	<input type="text" class="form-control" name="student_Number" value='<?php echo $student_Number;?>' placeholder="Staff ID" readonly>
            </div>
        </div>
    ...
    <div class="col-md-6">
        ...
        <div class="form-group">
            <label class="col-lg-4 control-label">Intake: </label>
            
            <div class="col-lg-8">
                <select class="form-control" name="intake_id">
                	<option>SELECT STUDENT INTAKE</option>
                	<?php
					$student_intakes = $this->reception_model->get_all_student_intakes();
                    	if($student_intakes->num_rows() > 0)
						{
							foreach($student_intakes->result() as $intakes)
							{
								$student_intake_id = $intakes->student_intake_id;
								$student_intake_name = $intakes->student_intake_name;
								
								if($student_intake_id == $intake_id)
								{
									echo '<option value="'.$student_intake_id.'" selected>'.$student_intake_name.'</option>';
								}
								
								else
								{
									echo '<option value="'.$student_intake_id.'">'.$student_intake_name.'</option>';
								}
							}
						}
					?>
...
<div class="center-align">
	<button class="btn btn-info btn-lg" type="submit">Edit Student Details</button>
</div>
<?php echo form_close();?>
    	</div>
    </div>
