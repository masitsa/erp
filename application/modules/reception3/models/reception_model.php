...
class Reception_model extends CI_Model 
{
 ...
 AND patients.dependant_id = \''.$strath_no.'\'');
	...
	*/
	...
	public function edit_student_patient($patient_id)
	{
		$data = array(
			'patient_phone1'=>$this->input->post('phone_number'),
			'patient_phone2'=>$this->input->post('patient_phone2'),
			'student_intake_id'=>$this->input->post('intake_id')
		);
	...
	*/
	...
	public function patient_names2($patient_id, $visit_id = NULL)
	{
		...
		foreach ($result->result() as $row)
		{
			...
			$close_card = 0;
			$student_intake_id = $row->student_intake_id;
		...
		if((!empty($student_intake_id))&&($student_intake_id > 0))
		{
			$cover_balance = $this->administration_model->patient_cover_balance($patient_id,$student_intake_id);
		}
		else
		{
			$cover_balance = 0;
		}
		$patient['cover_balance'] = $cover_balance;
		return $patient;
	}
	...
	*
	*/
	public function get_visit_departments($visit_id)
	{
		$this->db->from('visit_charge, service_charge , service, departments');
		$this->db->select('departments.*');
		$this->db->where('visit_charge.visit_id = '.$visit_id.' AND visit_charge.service_charge_id = service_charge.service_charge_id AND service_charge.service_id = service.service_id AND service.department_id = departments.department_id');
		$query = $this->db->get();
	...
	*/
	public function set_visit_department($visit_id, $department_id)
	{
		...
		{
			if(($department_id == 5) OR ($department_id == 7) OR ($department_id == 2) OR ($department_id == 4))
			{
				$accounts = 1;
			}
			else
	...
	*/
	...
	public function get_all_student_intakes()
	{
		$this->db->select('*');
		$query = $this->db->get('student_intake');
		return $query;
	}
	...
	public function save_staff_visit_payment_options($visit_id,$visit_payment_type)
	{//die();
		$visit_payment = array
			(
				'visit_id'=> $visit_id,
				...
				'staff_name'=>$this->input->post('staff_full_names'),
				'staff_faculty'=>$this->input->post('staff_faculty')
			);
		$this->db->insert('visit_payment',$visit_payment);
	}
	public function get_patient_file_number($patient_id)
	{
		...
		if($query->num_rows() > 0)
		{
			foreach($query->result() as $file_number)
			{
	...
	public function create_patient_file_number()
	{
		//select file numbetr
		//select product code
		$preffix = "SUMC";
		$this->db->from('patients');
		$this->db->where("file_number LIKE '%".$preffix."%'");
	...
	public function get_visit_departments2()
	{
	...
	public function get_todays_closed_visits()
	{
		$this->db->select('*');
		$this->db->where('visit_date = "'.date('Y-m-d').'" AND patients.patient_id = visit.patient_id');
		$query = $this->db->get('patients,visit');
